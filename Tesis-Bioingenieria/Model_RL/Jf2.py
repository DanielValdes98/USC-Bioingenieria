def Jf2(P):
    ...
    Jcp = (y_predT[1] - y_predT[2]) / (2*0.1*P[0])
    Jp = (y_predT[3] - y_predT[4]) / (2*0.1*P[1])
    Jk = (y_predT[5] - y_predT[6]) / (2*0.1*P[2])
    Jmi_h2o = (y_predT[7] - y_predT[8]) / (2*0.1*P[3])

    J = np.array((Jp, Jk, Jmi_h2o)).T
